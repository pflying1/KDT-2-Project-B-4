import React, { useEffect, useState } from 'react';
/**
 * 버스 위치를 나타내는 마커 모듈입니다.
 *
 * @param lati 위도입니다.
 * @param long 경도입니다.
 * @param map 마커를 지도에 표시합니다. map이라고 꼭넣어주세요.
 */

const busMarker = (lati:number,long:number,map:string|undefined) => {
  
  // const imageSrc = 'data:image/png;base64,', // 마커이미지의 주소입니다
  const imageSrc = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTQY9LzDY0aGnJsr4ptJZRO-auT8SYd3ma_fbSfFY3VgrztQ_jUEk60LA3Bpv5jjZ6xrkA&usqp=CAU', // 마커이미지의 주소입니다
  // const imageSrc = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQS91Nwu5c1YgQZuUnENqcJtWTtizjJcuVPvTgKFXMUEuNiQoWz--iOH6eyaV00umnkdm8&usqp=CAU', // 마커이미지의 주소입니다
  // const imageSrc = 'https://png.pngtree.com/png-vector/20190114/ourmid/pngtree-vector-school-bus-icon-png-image_314436.jpg', // 마커이미지의 주소입니다
  // const imageSrc = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRMN2ZKfkHodyg_0xdmehdtBYuOrvKX6S9R9wgxy0Ys3n0kqB9PKkoftaeM6728nr2QmK4&usqp=CAU', // 마커이미지의 주소입니다
  // const imageSrc = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSoJ2OcwbNnGNW_PwzAVRf5oTJFjyDPc6mOkdQICZodSCbPXV9lfhkdbq4vs5ThO18a3gY&usqp=CAU', // 마커이미지의 주소입니다    
  imageSize = new window.kakao.maps.Size(25, 25), // 마커이미지의 크기입니다
  imageOption = {offset: new window.kakao.maps.Point(8, 25)}; // 마커이미지의 옵션입니다. 마커의 좌표와 일치시킬 이미지 안에서의 좌표를 설정합니다.
    
  // 마커의 이미지정보를 가지고 있는 마커이미지를 생성합니다
  const markerImage = new window.kakao.maps.MarkerImage(imageSrc, imageSize, imageOption),
      markerPosition = new window.kakao.maps.LatLng(lati,long); // 마커가 표시될 위치입니다

  // 마커를 생성합니다
  const marker = new window.kakao.maps.Marker({
      position: markerPosition, 
      image: markerImage // 마커이미지 설정 
  });
  // 마커가 지도 위에 표시되도록 설정합니다
  marker.setMap(map);  
}

export default busMarker 

